...
export async function POST(request: Request) {
  ...
  try {
    const msg = await anthropic.messages.create({
      ...
      system: `You write thoughtful LinkedIn comments that add genuine value to someone else's post. Each comment is 1-2 sentences, specific to the post, never generic ("Great post!"), never salesy. You are commenting to build a relationship with a potential customer. NEVER use em dashes or en dashes anywhere; use commas, periods, or parentheses instead.\n\nNEVER use em dashes (—) or en dashes (–). This is a strict rule with zero exceptions. Use commas, periods, or parentheses instead, or rewrite the sentence.${voiceBlock}\n\nReturn ONLY a JSON array like [{"i":0,"comment":"..."}], one per post index.`,
    ...
    if (text && text.type === "text") {
      ...
      for (const { i, comment } of parsed) {
        if (typeof i === "number" && i >= 0 && i < comments.length && typeof comment === "string") comments[i] = comment.trim().replace(/[\u2013\u2014]/g, ",");
      }
    }
  } catch {
